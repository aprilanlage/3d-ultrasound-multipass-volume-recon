function out_frames_4 = pass_detect_phantom_r(poses_downsampled, bound_coords, xoffset, yoffset, UStoCam)
...
% assumes every other frame is segmented
...
% find maximum peaks based on minimum prominence 
...
% optional plotting
%scatter(fra_downsampled,x_poses); hold on
...
for k = 1:(length(out_frames_2)-1)
    if isnan(out_frames_2(k+1)) || isnan(out_frames_2(k))
        ...
        coTemp = surface_recon_r(poses_downsampled, bound_coords, (mid_pt-15):(mid_pt+15), xoffset, yoffset, UStoCam);
...
for g = 1:(length(out_frames_3)-1)
    if isnan(out_frames_3(g+1)) || isnan(out_frames_3(g))
        ...
        coTemp = surface_recon_r(poses_downsampled, bound_coords, p_1:p_2, xoffset, yoffset, UStoCam);
...
% out_frames_4 includes all checks applied
